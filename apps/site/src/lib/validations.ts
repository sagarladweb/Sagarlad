...
const realEmail = (msg: string) =>
  z
    .string()
    .trim()
    .email(msg)
    .toLowerCase()
    .refine((e) => !isRejectedEmail(e), { message: "Please enter a real email address" });

export const newsletterSchema = z.object({
  email: realEmail("Please enter a valid email address"),
  name: z.string().trim().min(2, "Please enter your name").max(80).optional().or(z.literal("")),
  acceptedTerms: z.boolean().optional(),
});

// Gate for eBook downloads: name + email (sanitized, real-only) and mandatory
// privacy/terms consent. The server never streams a file unless every field
// validates — the modal cannot be bypassed.
...
export const contactSchema = z.object({
  ...
  lastName: z
    ...
    .optional()
    .or(z.literal("")),
  email: realEmail("Please enter a valid email"),
  phone: z
    .string()
  ...
  organization: z
    ...
    .max(120)
    .regex(/^[\p{L}\p{M}\p{N}\s&'()/.,#\-]*$/u, "Organization contains invalid characters"),
