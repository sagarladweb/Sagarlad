...
export async function POST(
  ...
  try {
    ...
    // Audit log failure is non-critical — don't block the download
    logAudit("EBOOK_DOWNLOAD", {
      ip,
    ...
    if (subscribe) {
      ...
      if (existing) {
        ...
        await dbSafe(
          () => prisma.newsletterSubscriber.create({ data: { email, name, acceptedTerms: true } }),
          null
        );
      }
    }
